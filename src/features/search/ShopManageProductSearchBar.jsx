...
export default function ShopManageProductSearchBar() {
  ...
  return (
    <div className={`searchBar relative min-w-[400px]`}>
      <button onClick={() => dispatch(triggerSearch())} className="rounded-sm bg-neutral-200 hover:bg-neutral-300 active:bg-neutral-500 border border-neutral-400 absolute inset-y-1 right-1 w-12 flex items-center justify-center duration-200">
      ...
      <input value={searchText} onKeyDown={(e) => e.key === "Enter" ? dispatch(triggerSearch()) : ''} onChange={e => dispatch(updateSearchText(e.target.value))} type="text" id="search-navbar"
             className="block w-full p-2 pr-10 text-sm border text-neutral-700 border-neutral-400 rounded bg-white placeholder-neutral-400 outline-0" placeholder="Search product..."/>
    </div>
  )
}