...
export default function SearchBar({forMobile = false}) {
    ...
    return (
        <div className={`searchBar ${forMobile ? "md:hidden pt-4" : "hidden md:block"} relative min-w-[400px]`}>
            <Link to={`/search/${searchType.text}`} onClick={() => dispatch(triggerSearch())} className="bg-sky-400 hover:bg-sky-300 border border-neutral-500 absolute inset-y-1 right-1 w-12 flex items-center justify-center">
