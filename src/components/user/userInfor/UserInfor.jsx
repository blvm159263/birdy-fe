...
import { DatePicker, Form, Input, Select, Avatar, Button, Upload } from "antd"
...
function UserInfor({ isLoading }) {
  ...
  const onFinish = async (values) => {
    ...
    await userApi
      .isEmailExist(user.id, values.email)
      .then((response) => {
        console.log(response.data)
  ...
  const handleChangeAvatar = (info) => {
    userApi
      .uploadAvatar(user.id, { file: info.file.originFileObj })
      .then((response) => {
        console.log(response.data)
        setUser((prevState) => ({
          ...prevState,
